...
def player_register(request):
    ...
    query = """INSERT INTO posts_player (account_id, password, name, surname, nationality, age, kit_no, pref_foot, prev_transfer_fee, recovery_date, suspend_date, belong_to_team_name) 
		VALUES ('""" + account_id + "', '" + password + "', '" + name + "', '" + surname + "', '" + str(
        nationality) + "', "
    query2 = str(age) + ", " + str(kit_no) + ", '" + str(pref_foot) + "', " + str(prev_transfer_fee) + ", "
    query3 = "STR_TO_DATE('" + str(recdate) + "', '%d/%m/%Y'), " + "STR_TO_DATE('" + str(
        susdate) + "', '%d/%m/%Y'), '" + str(team) + "');"
...
def agent_register(request):
    ...
    query2 = account_id + "', '" + password + "', '" + name + "', '" + surname + "', '" + str(
        nationality) + "', '" + str(player) + "', " + str(age) + ");"
...
def leagues_info(request, league):
    cursor = connection.cursor()
    cursor.execute(
        ...
        "WHERE posts_team.league = '" + league + "'" +
